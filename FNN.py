...
# Create and load forward network (surrogate) to compare with surrogate of loki
...
# Initialize data structures to store info for plots
myplot = MyPlots() 
...
predict =  net_forward(net(train_dataset[:][0])).detach().numpy()
...
predict = scaler.inverse_transform(predict) # here I am inverse transforming the k's
target = y_data.numpy()
densities = x_data.numpy()
...
array = np.hstack((predict, scaler_max_abs.inverse_transform(densities)))
np.savetxt("C:\\Users\\clock\\Desktop\\Python\\predictions_test.txt", array, fmt= "%.12e") # attention to the format


# #---------------------------SAVE THE *TRAINING* PREDICTED K'S TO BE INSERTED IN THE SIMULATION AGAIN--------------------------------
# array = np.hstack((train_predictions, scaler_max_abs.inverse_transform(train_dataset[:][0].numpy())))
# np.savetxt("C:\\Users\\clock\\Desktop\\Python\\predictions_train.txt", array, fmt= "%.4e") # attention to the format