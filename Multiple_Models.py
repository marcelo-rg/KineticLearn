...
if __name__=='__main__':
    ...
    #--------------------Load the test dataset_______________________________________
    ...
    for i in range(nsamples):
        ...
        # T.manual_seed(random.randint(0,1000))
        # net = Net().to(device)
        # net.to(T.double) # set model to float64
        myplot = MyPlots() 
        # 5. Training algorithm
        print("Start training # %d"% (int(i+1)))
        for epoch in range(0, max_epochs):
            ...
            if (epoch % ep_log_interval) == 0:
                ...
                # print("epoch = %4d   loss = %0.4f   validation_loss= %0.4f,   = %0.4f" % \
        ...
        if i == 0:
            stack = net(train_dataset[:][0])
            stack_test = net(x_data)
            # mean_rel_err.append(rel_err(stack_test, target).mean(dim=0))
        else:
            predict = net(train_dataset[:][0])
            predict_test = net(x_data)
            # mean_rel_err.append(rel_err(predict_test, target))
            if i == 1:
                stack = T.stack((stack,predict))
                stack_test = T.stack((stack_test,predict_test))
            else:
                stack = T.cat((stack, T.reshape(predict, (1, train_size, len(k_columns)))), dim=0)
                stack_test = T.cat((stack_test, T.reshape(predict_test, (1, 100, len(k_columns)))), dim=0)
    ...
    train_predictions = T.mean(stack, dim=0).detach().numpy()
    var = T.var(stack, dim=0).detach().numpy()
    # Save stacks
    dir_path = "C:\\Users\\clock\\Desktop\\Python\\Images\\statistics\\stacks\\"
    T.save(stack, dir_path+ 'stack_train.pt')
    T.save(stack_test, dir_path+ 'stack_test.pt')
    y_train = train_dataset[:][1].numpy()
    np.save(dir_path+ "training_targets", y_train)
    np.save(dir_path+ "test_targets", target)
    np.save(dir_path+ "densities_targets", densities)
    np.save(dir_path+ "densities_targets_train", train_dataset[:][0].detach().numpy())
    ...
    # predict =  net(x_data).detach().numpy()
    predict = T.mean(stack_test, dim=0).detach().numpy()
    target = y_data.numpy()
    densities = x_data.numpy()
