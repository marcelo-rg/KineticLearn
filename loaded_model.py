...
class BayesianNet(nn.Module):
    def __init__(self):
        ...
        self.hid1 = bnn.BayesLinear(prior_mu=0, prior_sigma=0.5,
    ...
    def forward(self, x):
        ...
        z = T.cat((z[:,(0,1,2)],T.abs(z[:,(3,4,5)])),-1)
...
# def total_variance(T, x_tensor, target):
#   predictions = []
#   for t in range(T):
#     current_prediction = net(x_tensor).detach().numpy()
#     predictions.append(current_prediction[:,(0,1,2)]) 

#   print(predictions)
#   predictions = np.vstack((predictions[0] , predictions[1]))
  
#   return predictions

# predictions = total_variance(5, train_dataset[:][0], train_dataset[:][1].numpy())
# print(predictions)
