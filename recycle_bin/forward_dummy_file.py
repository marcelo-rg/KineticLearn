...
class Net(nn.Module):
    ...
    def forward(self, x):
        ...
        z = T.abs(self.oupt(z))  # no activation
        # z = T.relu(self.oupt(z))  # with activation
        return z
...
def variance_loss(output, target):
...
for epoch in range(0, max_epochs):
    ...
    for (batch_idx, batch) in enumerate(train_loader):
      ...
      # print("Output of %d iteration:"%epoch)
      # print(oupt.detach().numpy())
      # if(epoch == 12):
      #   exit()  
...
test_file = 'C:\\Users\\clock\\Desktop\\Python\\datapoints_test.txt'
...
for idx in range(len(predict[0])):
    ...
    # plt.plot(np.arange(1,len(predict)+1,1), sorted_ab[:,1], 'ro', label='predicted')
    # plt.plot(np.arange(1,len(predict)+1,1),sorted_ab[:,0], 'bo', label= 'target')
    plt.plot(np.arange(1,len(predict)+1,1), b, 'ro', label='predicted')
    plt.plot(np.arange(1,len(predict)+1,1),a, 'bo', label= 'target')
    plt.legend()
...
species = ['O2(X)','O2(a)', 'O(3P)']
my_x = net(x_data)
predict =  net_forward(my_x).detach().numpy()
# predict = scaler.inverse_transform(predict)
target = x_data.numpy()
# densities = x_data.numpy()
for idx in range(len(predict[0])):
...
array = np.hstack((scaler.inverse_transform(predict),densities*2.56e22))
