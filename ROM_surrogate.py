...
class LoadDataset(T.utils.data.Dataset):
  ...
  def __init__(self, src_file, m_rows=None):
    ...
    tmp_x = all_xy[:,[0,1,2]] # rate coefficients
    tmp_y = all_xy[:,[9,10,11]] 
    
    tmp_x = divide_and_extract(tmp_x)
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.hid1 = nn.Linear(2,100)  # hidden 1
...
def save_checkpoint(state, filename= "checkpoint_forward_k1k2k3_ROM.pth.tar"):
...
#Evaluation on a 100 point test set:
